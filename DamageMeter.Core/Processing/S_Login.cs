...
namespace DamageMeter.Processing
{
    internal class S_LOGIN
    {
        internal S_LOGIN(LoginServerMessage message)
        {
            ...
            {
                NetworkController.Instance.RaiseConnected(
                ...
                if (NetworkController.Instance.EntityTracker != null)
                {
                    try
                    {
                        var oldregion = BasicTeraData.Instance.Servers
                            .GetServer(NetworkController.Instance.EntityTracker.MeterUser.ServerId).Region;
                        trackerreset = NetworkController.Instance.Server.Region != oldregion;
                    }
                    catch
                    {
                        BasicTeraData.LogError(
                            "New server:" + NetworkController.Instance.Server + ";Old server Id:" + NetworkController
                ...
                if (trackerreset)
                {
                    NetworkController.Instance.TeraData =
                        BasicTeraData.Instance.DataForRegion(NetworkController.Instance.Server.Region);
                    ...
                    NetworkController.Instance.EntityTracker = new EntityTracker(BasicTeraData.Instance.MonsterDatabase,
                        NetworkController.Instance.UserLogoTracker);
                    NetworkController.Instance.PlayerTracker =
                        new PlayerTracker(NetworkController.Instance.EntityTracker, BasicTeraData.Instance.Servers);
            ...
            NetworkController.Instance.AbnormalityTracker = new AbnormalityTracker(
                NetworkController.Instance.EntityTracker, NetworkController.Instance.PlayerTracker,
                BasicTeraData.Instance.HotDotDatabase, NetworkController.Instance.AbnormalityStorage,
                DamageTracker.Instance.Update);
            if (NetworkController.Instance.MessageFactory.ChatEnabled)
            {
                NetworkController.Instance.AbnormalityTracker.AbnormalityAdded +=
                    NotifyProcessor.Instance.AbnormalityNotifierAdded;
                NetworkController.Instance.AbnormalityTracker.AbnormalityRemoved +=
                    NotifyProcessor.Instance.AbnormalityNotifierRemoved;
            }
            NetworkController.Instance.OnGuildIconAction(
                NetworkController.Instance.UserLogoTracker.GetLogo(message.PlayerId));
            NetworkController.Instance.EntityTracker.Update(message);
            BasicTeraData.Instance.EventsData.Load(NetworkController.Instance.EntityTracker.MeterUser.RaceGenderClass
                .Class);
