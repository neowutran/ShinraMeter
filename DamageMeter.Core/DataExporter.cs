    ...
    public class DataExporter
    {
        ...
        private static void SendAnonymousStatistics(string json, int numberTry)
        {
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var response = client.PostAsync("https://neowutran.ovh/storage/store.php", new StringContent(json, Encoding.UTF8, "application/json"));
        ...
        private static void ToTeraDpsApi(EncounterBase teradpsData, NpcEntity entity)
        {
            ...
            if (
                ...
                //areaId != 768 &&
                //areaId != 468 &&
                areaId != 770 && areaId != 769 && areaId != 916 && areaId != 969 && areaId != 970 && areaId != 710 && areaId != 780 && areaId != 980 &&
                areaId != 781 && areaId != 981 && !(areaId == 950 && int.Parse(teradpsData.bossId) / 100 != 11)) { return; }
            ...
            {
                Debug.WriteLine("Get server time error");
                NetworkController.Instance.BossLink.TryAdd("!0 " + LP.TeraDpsIoApiError + " " + entity.Info.Name + " " + entity.Id + " " + DateTime.UtcNow.Ticks,
                    entity);
                return;
            }
        ...
        private static void DELETEME(string json)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    client.Timeout = TimeSpan.FromSeconds(40);
                    var response = client.PostAsync("http://teralogs.com/api/logs", new StringContent(json, Encoding.UTF8, "application/json"));
                    var responseString = response.Result.Content.ReadAsStringAsync();
                    Debug.WriteLine(responseString.Result);
                }
            }
        ...
        {
            var url = server == 0 ? "https://moongourd.com/dpsmeter_data.php" : BasicTeraData.Instance.WindowData.PrivateDpsServers[server - 1];
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    {
                        NetworkController.Instance.BossLink.TryAdd(
                            "!" + server + " " + (string) responseObject["message"] + " " + boss.Info.Name + " " + boss.Id + " " + DateTime.UtcNow.Ticks, boss);
                    }
                }
            }
            catch (Exception e)
